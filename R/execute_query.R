...
execute_query <- function(
  ...
  if (should_paginate_query(return_default, field, variables)) {
...
should_paginate_query <- function(return_default, field, variables) {
  identical(return_default, field[[1]]) &&
    purrr::pluck_exists(variables, "filter", "per_page") &&
    purrr::pluck(variables, "filter", "per_page") == -1
}

execute_paginated_query <- function(
  query,
  variables,
  connection,
  return_default,
  ...
  first_data <- if (identical(response, "object")) first_response$data else first_response
  count <- response_metadata(first_data)$count
  first_page <- purrr::pluck(first_data, !!!field)

  if (count < rows_per_batch) {
    return(if (identical(response, "object")) {
      new_stashapi_response(first_page, first_response$meta, first_response$raw)
    } else {
      first_page
    })
  }

  total_pages <- max(1L, ceiling(count / rows_per_batch))
  ...
  other_pages <- fetch_remaining_pages(
    query = query,
    variables = variables,
    connection = connection,
    return_default = return_default,
  ...
  if (identical(response, "object")) {
    ...
    return(new_stashapi_response(
      combine_response_pages(page_values),
      modifyList(first_response$meta, list(pages = length(page_values))),
      first_response$raw
    ))
  }

  combine_response_pages(c(list(first_page), other_pages))
}

fetch_remaining_pages <- function(
  query,
  variables,
  connection,
  return_default,
  ...
  purrr::map(seq.int(2L, total_pages), function(page) {
    page_variables <- purrr::assign_in(variables, list("filter", "page"), page)
...
fetch_response <- function(
  ...
  if ("errors" %in% names(parsed) && length(parsed$errors) > 0L) {
    stop(new_stashapi_error(
      ...
      paste(
        ...
        format_graphql_errors(parsed$errors)
      ),
  ...
  if (purrr::pluck_depth(parsed) <= 1) {
    return(parsed)
  }
  data <- parsed$data
  if (purrr::pluck_exists(data, 1)) {
    data <- clean_response_data(purrr::pluck(data, 1))
  }

  if (
    (tibble::is_tibble(data) || all(purrr::map_lgl(data, tibble::is_tibble))) &&
      all(is.na(field))
  ) {
    return(if (identical(response, "object")) {
  ...
  if (all(!is.na(field))) {
    ...
    data <- purrr::map(data, function(value) {
      if (length(value) > 1 || is.data.frame(value)) {
        list(value)
      } else {
        value
      }
  ...
  if (identical(response, "object")) {
    return(new_stashapi_response(selected, response_metadata(data), parsed))
  }
  selected
}
...
validate_response_mode <- function(response) {
  if (
    !is.character(response) || length(response) != 1L ||
...
combine_response_pages <- function(pages) {
  ...
  if (all(purrr::map_lgl(pages, is.character))) {
    return(unlist(pages))
  }
...
#' Clean response
#' @description Internal function normalizes nested response data frames and
#'   replaces NULL and empty lists with NA.
#'
#' @param x list.
#'
...
#' @noRd
clean_response_data <- function(x) {
  if (is.data.frame(x)) {
    x <- dplyr::mutate(x, dplyr::across(dplyr::where(~ is.list(.x) || is.data.frame(.x)), ~ {
      ...
      if (is.data.frame(col)) {
        ...
        ncol(col) > 0,
        purrr::map_lgl(col, ~ is.null(.x) || is.data.frame(.x))
      )) {
    ...
    if (tibble::is_tibble(x)) {
      ...
      purrr::map(x, clean_response_data)
    }
  } else {
    x
  }
